import { BookingService } from '../services/bookServices';
  ...
  static bookARoom = async (req, res) => {
    ...
    if (room) {
      ...
      if (roomAvailable) {
        ...
        try {
          const trip = await BookingService.tripApproved(tripId);
          ...
          if (trip) {
            ...
            if (booked) {
              ...
              return res
                .status(201)
                .json({ status: 201, success: true, booked, message: 'Room booked successfully!' });
            }
            return res
              .status(400)
              .json({ status: 400, success: false, message: 'take care on dates' });
          }
          return res
            .status(403)
            .json({ status: 403, success: false, message: 'Trip need to be approved!' });
        } catch (error) {}
      }
      return res
        .status(400)
        .json({ status: 400, success: false, message: 'Room has been already booked!' });
    }
  ...
  static freeRoom = async (req, res) => {
    ...
    if (room) {
      ...
      if (roomBooked) {
        ...
        return res
          .status(200)
          .json({ status: 200, room, message: 'This room is set to free now.' });
      }
      return res
        .status(200)
        .json({ success: false, message: 'This room is available, no need to free it.' });
    }
