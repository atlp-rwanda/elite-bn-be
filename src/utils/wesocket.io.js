...
import { userJoin, getCurrentUser, userLeave, getRoomUsers } from './chat/users';
...
io.on('connection', async (socket) => {
  ...
  socket.on('joinRoom', async () => {
    ...
    socket.broadcast
      .to(user.room)
      .emit('message', formatMessage(chatName, `${user.firstname} has joined the chat`));
  ...
  socket.on('chatMessage', async (msg) => {
    ...
    io.to(user.room).emit('message', formatMessage(user.room, msg, firstname, lastname));
