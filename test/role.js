...
describe('TEST ROLES.', () => {
  ...
  it('should login as normal user not super admin', async () => {
    const res = await request(app).post('/api/v1/user/login').send(notSuperAdmin);
  ...
  it('should login as super administrator', async () => {
    const res = await request(app).post('/api/v1/user/login').send(loginAdmin);
    expect(res).to.have.status(200);
    expect(res.body).to.have.property('token');
    expect(res.body).to.have.property('message', 'User logged in successfully');
  ...
  it('should be allowed to get all registed user', async () => {
    const res = await request(app).get('/api/v1/user/allUser').set('Cookie', `jwt=${adminAuth}`);
    expect(res).to.have.status(200);
